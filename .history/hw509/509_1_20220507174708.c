...
void DFS(Node* node, int resultOfDfs[], int* k){
   ...
   void helper(Node* node){
      ...
      if(node != NULL &&node->left != NULL) helper(node->left);
      else {
         resultOfDfs[*k] = 0;
         *k = *k + 1;
      }
      ...
      *k = *k + 1;
      ...
      if(node != NULL &&node->right != NULL) helper(node->right);
      else {
         resultOfDfs[*k] = 0;
         *k = *k + 1;
      }
...
void appendRecursively(Node* root, int nodeVals[], Array* arr, int* j){
   ...
   if(direction == 1){
      ...
      if(root != NULL)
         appendRecursively(root->left, nodeVals, arr, j);
   }else if(direction == 2){
      ...
      if(root != NULL)
         appendRecursively(root->right, nodeVals, arr, j);
   }
}
